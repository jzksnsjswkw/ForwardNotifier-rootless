...
static void loadPrefs() {
  NSMutableDictionary *prefs = [NSMutableDictionary dictionaryWithContentsOfFile:ROOT_PATH_NS(@"/User/Library/Preferences/com.greg0109.forwardnotifierprefs.plist")];
    ...
    if ([[[NSUserDefaults standardUserDefaults] stringForKey:@"ForwardNotifier-Status"] isEqual:@"1"] && (locked)) {
      ...
      dispatch_async(sendnotif, ^{
        ...
        if (!jsonData) {
            ...
            // 将JSON数据转换成NSString
            command = [[NSString alloc] initWithData:jsonData encoding:NSUTF8StringEncoding];
        }
        ...
        NSURLSessionDataTask *dataTask = [session dataTaskWithRequest:request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
          if (error) {
            NSLog(@"%@", error);
            if (errorlog) {
              testnotif(@"ForwardNotifier Error", [NSString stringWithFormat:@"%@", error]);
            }
          }
        }];
        [dataTask resume];
...
-(void)publishBulletin:(BBBulletin *)arg1 destinations:(unsigned long long)arg2 {
  ...
  if ([bundleID length] != 0) {
    BBSectionInfo *sectionInfo = [notificationserver _sectionInfoForSectionID:bundleID effective:YES];
    NSLog(@"%@", sectionInfo);
